...
#STEP 3: Average for scores is calulated and passed to dictionary with key "Avg."
...
def create_record(temp_record):
    ...
    if 'Test_1' not in temp_record:
        ...
        grades_dict["Test_1"] = int(temp_record[temp_record.index('Test_1')+1])

    if 'Test_2' not in temp_record:
        grades_dict["Test_2"] = 0
    else:
        grades_dict["Test_2"] = int(temp_record[temp_record.index('Test_2') + 1])


    if 'Test_3' not in temp_record:
        grades_dict["Test_3"] = 0
    else:
        grades_dict["Test_3"] = int(temp_record[temp_record.index('Test_3')+1])

    if 'Test_4' not in temp_record:
        grades_dict["Test_4"] = 0
    else:
        grades_dict["Test_4"] = int(temp_record[temp_record.index('Test_4')+1])
    #STEP 3 ends
...
def write_record(temp_record):
    print("{0:^10s}|{1:^10s}|{2:^10d}|{3:^10d}|{4:^10d}|{5:^10d}|{6:^10.2f}"
          .format(temp_record['ID'], temp_record['Name'],temp_record['Test_1'],
                  temp_record['Test_2'], temp_record['Test_3'],temp_record['Test_4'],
                  temp_record['Avg.']))
...
## internaly calls 'create_record' to create each line to be written on console
def print_grades(input_file):
    grades_file = open(input_file, 'r')
    flag = True
    print("{0:^10s}|{1:^10s}|{2:^10s}|{3:^10s}|{4:^10s}|{5:^10s}|{6:^10s}"
          .format('ID', 'Name', 'Test_1', 'Test_2', 'Test_3', 'Test_4','Avg')) #STEP 2
    while(flag):
